...
function getMovies() {
    fetch(`https://api.themoviedb.org/3/movie/now_playing?api_key=${key}`)
        ...
        .then((data) => {
            data.results.forEach(element => {
                document.querySelector("#div1").insertAdjacentHTML("beforeend", `<img data-id=${element.id} src="${poster}${element.poster_path}" width="150px" style="padding: 10px">`)
            });
        })
    ...
    fetch(`https://api.themoviedb.org/3/movie/popular?api_key=${key}`)
        ...
        .then((data) => {
            data.results.forEach(element => {
                document.querySelector("#div2").insertAdjacentHTML("beforeend", `<img data-id=${element.id} src="${poster}${element.poster_path}" width="150px"style="padding: 10px">`)
            });
        })
    ...
    fetch(`https://api.themoviedb.org/3/movie/upcoming?api_key=${key}`)
        ...
        .then((data) => {
            data.results.forEach(element => {
                document.querySelector("#div3").insertAdjacentHTML("beforeend", `<img data-id=${element.id} src="${poster}${element.poster_path}" width="150px" style="padding: 10px">`)
            });
        })
}
...
function search() {
    document.querySelector("#goSearch").addEventListener("click", function() {
        ...
        <div id="div0" class="posters">
            <h1 style="color:#fff">results of "${search}":</h1>
        </div>
        `)
            ...
            .then((data) => {
                data.results.forEach(element => {
                    if (element.poster_path != undefined) {
                        document.querySelector("#div0").insertAdjacentHTML("beforeend", `<img data-id=${element.id} src="${poster}${element.poster_path}" width="150px" style="padding: 10px">`)
                    }
                });
            })
            ...
            fetch(`https://api.themoviedb.org/3/movie/${element.dataset.id}/videos?api_key=${key}`)
                ...
                .then((data) => {
                    ...
                    `)
                        ...
                        alert("Sorry, we still don't have video for this film!")
                    }
                })
        })
    })
